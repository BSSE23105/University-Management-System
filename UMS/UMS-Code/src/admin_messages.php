...
if (isset($_POST['delete_message'])) {
    ...
    header("Location: admin_messages.php?tab=$tab");
    exit;
}
...
$inbox = $conn->query("
    ...
    WHERE m.receiver_id = $admin_id AND m.receiver_type = 'admin'
                        ...
                        <tbody id="inbox-container" class="bg-white divide-y divide-gray-200 <?= $active_tab === 'inbox' ? '' : 'hidden' ?>">
                            <?php if ($inbox->num_rows > 0): ?>
                                <?php while ($message = $inbox->fetch_assoc()): 
                                    ...
                                    $name = $sender_type === 'faculty' ? $message['faculty_name'] : $message['student_name'];
                                    $initials = substr($name, 0, 1) . substr($name, strpos($name, ' ') + 1, 1);
                                    $avatar_class = $sender_type === 'faculty' ? 'faculty-avatar' : 'student-avatar';
                                ?>
                        ...
                        <tbody id="sent-container" class="bg-white divide-y divide-gray-200 <?= $active_tab === 'sent' ? '' : 'hidden' ?>">
                            <?php if ($sent->num_rows > 0): ?>
                                <?php while ($message = $sent->fetch_assoc()): 
                                    ...
                                    $name = $receiver_type === 'faculty' ? $message['faculty_name'] : $message['student_name'];
                                    $initials = substr($name, 0, 1) . substr($name, strpos($name, ' ') + 1, 1);
                                    $avatar_class = $receiver_type === 'faculty' ? 'faculty-avatar' : 'student-avatar';
                                ?>
